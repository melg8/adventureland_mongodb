...
async function api_call_l(method,args,r_args)
{
	if(!args) args={};
	var result={};
	await api_call(method,args,r_args).then(function(data){ result=data; }).catch(function(data){ result=data; });
...
function handle_information(infs)
{
	...
	if(!Array.isArray(infs)){ infs=infs.infs||[]; }
			...
			else if(info.purpose=="save")
			{
				...
				if(c_slot)
					for(var num in info.list)
						if(""+num===""+c_slot) c_name=info.list[num][0];
					...
					if(character.ctype!="merchant")
						for(var num in ui_list)
						{
							if(parseInt(num)>100||(""+num).startsWith("CH_"))
								delete ui_list[num];
						}
		...
		setTimeout(function(num,fname){
			return function(){
				if(code_ref[num] && code_ref[num].file==fname) // make sure it's not renamed
					show_confirm("Are you sure you want to delete "+cdata.file+"?",["#D06631","Yes"],"No!",function(slot){return function(){ api_call("save_code",{name:"DELETE",slot:slot,electron:true}); hide_modal(true);}}(num));
			}
